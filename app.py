...
filterParams.add_argument('pmid', type=int, required=False, help='Pubmed ID of a requested publication.')
filterParams.add_argument('trait', type=str, required=False, help='Trait of ontology term.')
filterParams.add_argument('pvalue', type=str, required=False, help='Upper boundary of the p-value (eg. 1e-8).')
filterParams.add_argument('sample', type=str, required=False, help='Part of the sample description.')
filterParams.add_argument('ancestry', type=str, required=False, help='Broad ancestry description of the samples.')
...
class diagramFilter(Resource):
    ...
    def post(self):
        ...
        logging.info(f'Diagram filter. Parsed parameters: {json.dumps(parameters)}')
...
class dataRetriever(Resource):
    ...
    def post(self):
        ...
        # Parsing and validating input paramters
        parameters = eu.validate_paramters(filterParams.parse_args())
        logging.info(f'Association retriever. Parsed parameters: {json.dumps(parameters)}')
